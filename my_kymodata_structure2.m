function my_kymodata_structure2(color)
% Initialize
coloris = color;
kymoname = {}; 
kymofilename = {};
...
%REMOVE the next section after I fix the python script for this
for i = 1:length(kymoname) % Get name of kymos
    if contains(kymoname{i}, 'green')
        str = kymoname{i};
        match = ["green"];
        name = erase(str,match);
        kymoname{i} = name;
